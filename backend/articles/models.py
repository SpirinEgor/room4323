...
class ArticleManager(models.Manager):
    def getAllArticlesTitle(self, approved):
        query_set = self.filter(approved=approved)
        dictionary = {}
        for category in Category.objects.all():
            val = list(query_set.filter(category=category).values_list('title', 'slug'))
            if val == []:
                continue
...
class Article(models.Model):
    ...
    def toDict(self):
        return dict(author=self.author.username, title=self.title, rating=self.rating,
                    algorithm=self.algorithm, category=self.category.name, id=self.id,
                    created=str(self.created.strftime('%d-%m-%y')))
